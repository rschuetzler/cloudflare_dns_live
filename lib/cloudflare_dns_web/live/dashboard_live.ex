        ...
        if DNSValidator.can_modify_record?(record) do
          case CloudflareClient.delete_dns_record(id, %{name: record.name, type: record.type}) do
            :ok ->
              ...
              flash_message =
                "#{subdomain} (#{record.type}) record #{record.name} successfully deleted"
  ...
  def render(assigns) do
        ...
        <div class="px-4 py-6 mx-auto max-w-7xl sm:px-6 lg:px-8">
          <div class="flex justify-between items-center">
            <div>
              ...
              <p class="mt-1 text-sm text-gray-600">Manage DNS records for {zone_domain()} domain</p>
            </div>
            <div class="flex space-x-4">
              ...
              <form method="post" action="/logout" class="inline">
                ...
                >
                  <.icon name="hero-arrow-right-on-rectangle" class="-ml-1 mr-2 h-4 w-4" /> Logout
                </button>
              </form>
            </div>
          </div>
    ...
    <!-- Records Stats -->
        <div class="mb-6 text-sm text-gray-600">
          Showing <span class="font-medium">{length(@records)}</span>
          ...
          <%= if @records == [] do %>
            <div class="text-center py-12">
              ...
              <div class="mt-6">
                ...
                >
                  <.icon name="hero-plus" class="-ml-1 mr-2 h-5 w-5" /> Add DNS Record
                </.link>
              </div>
            </div>
          <% end %>
